        ...
        if '/pr_find' in msgText:
            try:
                try:
                    ...
                    time.sleep(1.5)
                    try:bot.editMessageText(chat_id=update.message.chat.id,message_id=id_msg,msgText=f"{msg_start}\n\n➖➖➖➖➖➖➖\nBuscando proxy...\n\n➖➖➖➖➖➖➖")
                    except Exception as ex:bot.sendMessage(update.message.chat.id,ex)
                    for port in range(int(rango_min),int(rango_max)):
                        ...
                        if result == 0:
                            ...
                            try:bot.editMessageText(chat_id=update.message.chat.id,message_id=id_msg,msgText=f"{msg}")
                            except Exception as ex:bot.sendMessage(update.message.chat.id,ex)
                            break
                        else:
                            pr_find=0
                    ...
                    try:
                        if pr_find==0:
                            bot.editMessageText(chat_id=update.message.chat.id,message_id=id_msg,msgText=f"🛰 No Hubo Éxito Buscando Proxy!!\n\n❌ IP : {ip}\n\n❌ PUERTOS : {rango_min}-{rango_max}")
                        else:pass
                    except Exception as ex:print(str(ex))
                    return
                except:
